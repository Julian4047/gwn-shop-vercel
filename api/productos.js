  ...
  return `
    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
      <div class="relative aspect-square overflow-hidden group">
        ...
        ${
          product.hasPromo
            ? `<div class="absolute top-3 left-3 bg-red-500 text-white px-3 py-1 rounded-full text-xs font-semibold">${product.promoText}</div>`
            : ""
        }
      </div>
      <div class="p-4 flex flex-col">
        ${
          product.variants.length > 0
            ? `<div class="flex gap-2 mb-3 flex-wrap justify-center">${variantsHtml}</div>`
            : ""
        }
        <div class="text-center mb-3">
          <span class="text-3xl font-bold text-gray-900">${product.price}</span>
        </div>
        <a href="javascript:void(0);" onclick="addToCart('${product.productName}', '${product.price}')" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg text-base font-semibold transition-colors duration-200 no-underline text-center w-full block mb-2">
          Agregar al carrito
        </a>
        <a href="${whatsappUrl}" target="_blank" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg text-base font-semibold transition-colors duration-200 no-underline text-center w-full block mb-3">
          Comprar
        </a>
        <h3 class="text-base font-semibold text-gray-700 text-center line-clamp-2">${product.productName}</h3>
      </div>
    </div>
  `;
}
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    try {
      ...
      for (const html of htmlArray) {
        ...
        if (productData.productName) products.push(productData);
      }
    } catch (err) {
  ...
  <body class="bg-white">
    <nav class="w-full bg-white shadow-md fixed top-0 left-0 z-50">
      <div class="container mx-auto px-4 py-3 flex justify-center">
        <h1 class="text-3xl font-normal font-poppins tracking-tight"><span class="text-[#353535]">GWN</span><span class="text-[#C23235]">.</span><span class="text-[#3235C2]">SHOP</span></h1>
      </div>
    </nav>
    ...
    <script>
      ...
      function renderCart() {
        ...
        itemsDiv.innerHTML = cart.map((item, i) => \`<div class="flex justify-between items-center"><span>\${item.name} - \${item.price}</span><button onclick="removeFromCart(\${i})" class="text-red-500 font-bold px-2">X</button></div>\`).join('');
      }
      document.getElementById('checkout-btn').addEventListener('click', () => {
        if (cart.length === 0) return;
        const message = \`Hola! Quisiera comprar los siguientes productos:\\n\\n\` + cart.map(item => \`\${item.name} - \${item.price}\`).join('\\n') + \`\\n\\nGracias!\`;
        window.open(\`https://wa.me/5491165756608?text=\${encodeURIComponent(message)}\`, '_blank');
      });
    </script>
  </body>
  </html>
